...
describe('Component: ProgramVisitEncounterSearch', () => {
  ...
  fit('reset filter should clear all filter data', () => {


    comp.departments = mockDepartmentsFilter;
    comp.program = programsSelected;
    comp.visitType = visitType;
    comp.visitTypes = visitTypes;
    comp.filterSet = true;
    ...
    expect(comp.department.length).toEqual(0);
    expect(comp.program.length).toEqual(0);
    expect(comp.visitType.length).toEqual(0);
    expect(comp.visitTypes.length).toEqual(0);
    expect(comp.filterSet).toBe(false);
  });
