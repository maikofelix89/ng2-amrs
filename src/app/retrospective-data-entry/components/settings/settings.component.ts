...
export class RetrospectiveSettingsComponent implements OnInit {
  ...
  public provider: any;
  public suggest = new Subject<string>();
  public location: any;
  ...
  private _init() {
    ...
    this.retrospectiveDataEntryService.retroSettings.subscribe((retroSettings) => {
      ...
      this.suggest.subscribe((term) => {
        if (!_.isNil(term) && term.length > 2) {
