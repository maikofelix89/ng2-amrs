...
export class RetentionReportComponent implements OnInit , OnChanges {
    ...
    public ngOnInit() {
        ...
        this.retentionReportService.getRetentionIndicatorDefinitions()
        .subscribe((result) => {
            if (result) {
...
public fetchReport() {
     this.loading();
     this.retentionReportService.getRetentionReport(this.params).pipe(
     take(1)).subscribe((result) => {
       this.retentionSummary = {
        'appointment_adherence': result['appointmentAdherence'].result,
        'defaulter_tracing': result['defaulterTracing'].result
       };
       this.endLoading();
     }, (err) => {
       this.endLoading();
